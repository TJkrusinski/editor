...
const CEILING_VERT: Array<[Vertex3D, Vertex3D]> = Array.from(
  ...
  (_, i) => {
    const x = map(i, 0, NUM_VERT_LINES, ROOM_LEFT_LINE_X, ROOM_RIGHT_LINE_X);
...
const LEFT_WALL_VERT = Array.from(
  { length: NUM_VERT_LINES },
  (_, i) => {
    const y = map(i, 0, NUM_VERT_LINES, ROOM_BOTTOM_LINE_Y, ROOM_TOP_LINE_Y);
...
const LEFT_WALL_HOR = Array.from(
  { length: NUM_HOR_LINES },
  (_, i) => {
    const z = map(i, 0, NUM_HOR_LINES, ROOM_LINES_Z, ROOM_LINES_Z_FOREGROUND);
...
const RIGHT_WALL_VERT = Array.from(
  { length: NUM_VERT_LINES },
  (_, i) => {
    const z = map(i, 0, NUM_VERT_LINES, ROOM_LINES_Z, ROOM_LINES_Z_FOREGROUND);
    return [{
      x: ROOM_RIGHT_LINE_X,
  ...
  useLayoutEffect(() => {
    ...
    lines.forEach((line) => {
      ...
      if (start && end) {
        ...
        const projectedStart = projectLine(rotatedStart, 10000, ORIGIN);
        const projectedEnd = projectLine(rotatedEnd, 10000, ORIGIN);
